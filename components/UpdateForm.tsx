...
type FormProps={
    ...
    setOpen: React.Dispatch<React.SetStateAction<updateForm>>,
    id:number
}

export default function UpdateForm({expensesData,setFilteredData,setOpen,id}:FormProps){

    let defaultValue:note=expensesData.find(note=>note.id==id)||{
    ...
    const handleClick=()=>{
        ...
        if(d1>=d2){
            ...
            let new_data=expensesData.filter(note=>note.id!=id)
            new_data.push(new_note)
            new_data = new_data.sort((a, b) => a.id - b.id);
    ...
    <div className=" w-full min-h-screen flex justify-center items-center bg-black/75 fixed top-0">
        <div className="flex justify-center items-center flex-col px-10 bg-white rounded-sm max-h-[calc(100svh-10px)] overflow-y-auto">
            <div className="mb-10 pt-6">
                <div className="flex  justify-center items-center">
                    <h1 className="font-semibold">Modifier votre note de frais</h1>
                    <ImCross onClick={()=>setOpen({id:0,isOpen:false})} className="translate-x-28 -translate-y-4 w-3 h-3 text-xs cursor-pointer"/>
                </div>
                <div className="space-y-4 mt-4 w-96">
                    ...
                    <div className="w-full flex justify-center items-center pt-6">
                        <Button disabled={Erreurs.length!=0} type="button" className="w-full cursor-pointer bg-green-400 hover:bg-green-600" onClick={handleClick}>Enregister modifications</Button>
                    </div>
                </div>
            </div>
        </div>
